...
def build_datasets(p:EasyDict)->EasyDict:
    ...
    source_ds = ORACLE_Torch.ORACLE_Torch_Dataset(
                    ...
                    num_examples_per_device=p.num_examples_per_class_per_domain*len(p.source_domains),
                    seed=p.dataset_seed,  
    ...
    target_ds = ORACLE_Torch.ORACLE_Torch_Dataset(
                    ...
                    window_length=p.window_length,
                    window_stride=p.window_stride,
                    num_examples_per_device=p.num_examples_per_class_per_domain*len(p.target_domains),
                    seed=p.dataset_seed,  
                    max_cache_size=p.max_cache_items,
                    transform_func=lambda x: (x["iq"], serial_number_to_id(x["serial_number"]), x["distance_ft"]),
                    prime_cache=False
